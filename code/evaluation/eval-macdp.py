...
def save_df(env_df, s_dir, s_quote):
    s_ver = 0
    while True:
        if os.path.isfile(f"{s_dir}eval_{s_quote}_{s_ver}.csv"):
            s_ver += 1
        else:
            env_df.to_csv(f"{s_dir}eval_{s_quote}_{s_ver}.csv", index=False)
            break
...
def save_action(mcs_start_hex, mcs_actions, s_quote):
    ...
    s_ver = 0
    while True:
        if os.path.isfile(f"{s_dir}eval_{s_quote}_{s_ver}.txt"):
            s_ver += 1
        else:
            with open(f"{s_dir}eval_{s_quote}_{s_ver}.txt", 'w') as file:
                file.write(f"{dic}")
            break

def eval_vdn(agents, env, s_dir, s_qoute, episodes=1, epsilon=0.0):
    episode_rewards = []
    episode_steps = []
    episode_demands = []
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            next_states = torch.from_numpy(next_state_np).float()
            rewards = torch.from_numpy(R_total_np).float().unsqueeze(1)
            states = next_states
...
if __name__ == "__main__":
    ...
    logger.add(f"./log/eval_{algo}/{s_quote}_{datetime.now().strftime('%Y%m-%d_%H:%M:%S')}.log", format=fmt, rotation="10 MB", enqueue=True, backtrace=True)
    ...
    # Train the
    episode_rewards, episode_steps, episode_demands, episode_perfect_scores, episode_utilizations = eval_vdn(agents, env, s_dir, s_quote)
