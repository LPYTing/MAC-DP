...
class Agent:
    ...
    def select_action(self, state, epsilon):
        ...
        if torch.rand(1).item() < epsilon:
            ...
            # Normalize Q-values for valid actions
            valid_q_values = torch.exp(q_values - torch.max(q_values))  # Apply softmax-like stability
            valid_q_values[~mask] = 0  # Invalidate non-valid Q-values

            # Add custom probability bias
            custom_probabilities = torch.zeros_like(q_values, dtype=torch.float).to(device)
            for i in valid_actions:
                if 0 <= i <= 6:
                    custom_probabilities[i] = 0.3
                elif i > 6:
                    custom_probabilities[i] = 0.7
                else:
                    print("ERROR: Check the index number.")
                    exit(1)

            # Normalize custom probabilities
            custom_probabilities /= custom_probabilities.sum()

            # Combine Q-values and custom probabilities
            weighted_probabilities = valid_q_values * custom_probabilities
            weighted_probabilities /= weighted_probabilities.sum()  # Normalize to form a probability distribution

            # Sample an action based on the weighted probabilities
            action_distribution = Categorical(weighted_probabilities)
            selected_action = action_distribution.sample()

            return selected_action.item()  # assuming you want to return the index as a Python number
...
if __name__ == "__main__":
    ...
    s_quote = f"{algo}_ver_hex_map6v3_{period}_lr{lr}_epi{episodes}_prob{prob_dist}_lb{lambda1}_epsilon_decay{epsilon_decay}-{N}"
