...
class MobileChargingEnvironment:
    ...
    def step(self, actions):
        ...
        global f_K
        R_total_np = np.array(R_m_list) + (f_K * R_f.sum() / len(self.MCS))
        logger.debug(f"Period: {self.timestamp.strftime('%Y-%m-%d_%H:%M:%S')} -> R_total={R_total_np.sum()}, f_K={f_K}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 3:
        ...
        f_K = 0.1
    lambda1 = 0.2
    logger.debug(f"lambda1={lambda1},_n={_n},_epsilon_decay={_epsilon_decay},_lr={_lr},f_K={f_K}")
    ...
    s_quote = f"{algo}_ver_fK{f_K}_{period}_lr{lr}_epi{episodes}_prob{prob_dist}_lb{lambda1}_epsilon_decay{epsilon_decay}-{N}"
