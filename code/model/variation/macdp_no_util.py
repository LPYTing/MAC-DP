...
class MobileChargingEnvironment:
    ...
    def step(self, actions):
        ...
        for mcs in self.MCS:
            ...
            R_m = 1 / (1 + np.exp(self.lambda1 * perfect_scores[mcs.id]))
            R_m_list.append(R_m)
        ...
        R_f = 1 / (1 + np.exp(self.lambda1 * f_perfect))
...
if __name__ == "__main__":
    ...
    s_quote = f"{algo}_ver_delUtil_{period}_lr{lr}_epi{episodes}_prob{prob_dist}_lb{lambda1}_epsilon_decay{epsilon_decay}-{N}"
